...
const todayMenu1Lunch = require("./todayMenu1Lunch");
const todayMenu2Breakfast = require("./todayMenu2Breakfast");
const todayMenu2Lunch = require("./todayMenu2Lunch");
const todayMenu2Dinner = require("./todayMenu2Dinner");
  ...
  switch (content) {
    case "🎁뭐먹지":
      ...
      if (utils.getTime() == "morning") {
        ...
        callback(
          null,
          message.baseType("밤에는 그냥 집에서 치킨 시켜 드세요.(하하)")
        );
      }
    ...
    case "1식당-점심":
      if (utils.getDay() == "Sunday" || utils.getDay() == "Saturday") {
        ...
        if (cache.get(content)) {
          ...
          todayMenu1Lunch(utils.formatDate(), function(data) {
            callback(null, message.baseType(data));
            cache.put(content, data, 6 * 60 * 60 * 1000);
          });
        }
    ...
    case "2식당-아침":
      if (utils.getDay() == "Sunday" || utils.getDay() == "Saturday") {
        ...
        if (cache.get(content)) {
          ...
          todayMenu2Breakfast(utils.formatDate(), function(data) {
            callback(null, message.baseType(data));
            cache.put(content, data, 6 * 60 * 60 * 1000);
          });
        }
    ...
    case "2식당-점심":
      if (utils.getDay() == "Sunday" || utils.getDay() == "Saturday") {
        ...
        if (cache.get(content)) {
          ...
          todayMenu2Lunch(utils.formatDate(), function(data) {
            callback(null, message.baseType(data));
            cache.put(content, data, 6 * 60 * 60 * 1000);
          });
        }
    ...
    case "2식당-저녁":
      if (utils.getDay() == "Sunday" || utils.getDay() == "Saturday") {
        ...
        if (cache.get(content)) {
          ...
          todayMenu2Dinner(utils.formatDate(), function(data) {
            callback(null, message.baseType(data));
            cache.put(content, data, 6 * 60 * 60 * 1000);
          });
        }
