...
function todayMenu2Breakfast(date, callback) {
  if (cache.get("todayMenu2Breakfast" + date)) {
    console.log("Data from cache, " + date);
    var html = cache.get("todayMenu2Breakfast" + date);

    var $ = cheerio.load(html);
    var returnString = "";
    returnString = "<2식당(DE타워) - 아침> (하하)";
    ...
    request.post(
      {
        url: `http://mydish.welstory.com/todaymenu.do?restaurantCode=REST000049&toDay=${date}&mealType=1`,
        encoding: "utf-8"
      },
      function(error, response, html) {
        ...
        cache.put("todayMenu2Breakfast" + date, html, 1 * 60 * 60 * 1000);
...
module.exports = todayMenu2Breakfast;
